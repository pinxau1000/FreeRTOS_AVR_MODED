...
/*-----------------------------------------------------------*/
void vPrint1String( const char *pcString )
{
	/* Print the string, suspending the scheduler as method of mutual
	exclusion. */
	vTaskSuspendAll();
	{
      Serial1.print(pcString);
      Serial1.flush();
	//	printf( "%s", pcString );
	//	fflush( stdout );
	}
...
void vPrint1StringAndNumber( const char *pcString, unsigned portLONG ulValue )
{
	/* Print the string, suspending the scheduler as method of mutual
	exclusion. */
	vTaskSuspendAll();
